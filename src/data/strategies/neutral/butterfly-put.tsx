...
const strategy: Strategy = {
  id: 'butterfly-put',
  name: '看跌蝶式价差 (Long Put Butterfly)',
  referencePrice: 100,
  tags: ['方向：中性', '支付权利金', '风险/收益：有限'],
  summary: '买入一份低执行价和一份高执行价的看跌期权，同时卖出两份中间执行价的看跌期权，以博取股价在目标价位附近盘整的收益。',
  description: '看跌蝶式价差是一种中性策略，适用于预期股价在未来一段时间内波动性很小，会稳定在某个特定价格附近的情况。它通过构建一个“帐篷”形的盈亏曲线，在中间执行价达到最大利润。如果股价大幅上涨或下跌，亏损将被限制在最初支付的少量净权利金内。',
  ...
  example: (
    <div>
      <p>你预测M公司股票（当前股价$100）在一个月后会非常接近$100。于是你建立了一个看跌蝶式价差：</p>
      <ul>
        <li>买入1张执行价为$105的看跌期权，花费$3/股（共$300）。</li>
        <li>卖出2张执行价为$100的看跌期权，每张收入$2/股（共$400）。</li>
      ...
      <p>你买入的$105看跌期权盈利$500，卖出的$100看跌期权价值为零，买入的$95看跌期权也价值为零。你的净利润为 $500 - $100 (成本) = <strong>$400</strong>，这是最大利润。</p>
      <p><strong>情景二：股价到期时收于$105或$95之外</strong></p>
      <p>所有期权都将变为废纸或相互抵消，你将损失全部初始投入的 <strong>$100</strong>。</p>
      <p><strong>盈亏平衡点</strong></p>
      <p>此策略有两个盈亏平衡点：<em>$105 - $1 = $104</em> 和 <em>$95 + $1 = $96</em>。股价在$96到$104之间是你的盈利区间。</p>
    </div>
  ),
  formula: [
  ...
  legs: [
    ...
    { id: 'sp100x2', kind: 'option', position: 'short', qty: 2, multiplier: 100, entryPrice: 2, option: { type: 'put', strike: 100 } },
