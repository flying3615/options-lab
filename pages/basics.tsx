...
export default function Basics() {
  return (
    <section>
      ...
      <div className={styles.greeksGrid}>
        <div className={styles.greekCard}>
          <h3><Link href={"/strategies/long-call"}>买入看涨期权 (Long Call)</Link></h3>
          <p>
            最纯粹的看涨方式。如果你坚信一支股票会大涨，这是一个用较小成本博取巨大收益的理想工具。
          </p>
          <p><strong>适合场景：</strong>预期股价将有大幅上涨时。</p>
          <p><Link href={"/strategies/long-call"} className="button">学习 Long Call策略 &rarr;</Link></p>
        </div>
        <div className={styles.greekCard}>
          <h3><Link href={"/strategies/long-put"}>买入看跌期权 (Long Put)</Link></h3>
          <p>最纯粹的看跌或保护方式。如果你认为一支股票会大跌，或者想为你的持仓买一份“保险”，它能帮你从价格下跌中获利。</p>
          <p><strong>适合场景：</strong>预期股价将有大幅下跌，或为已有持仓提供保护。</p>
          <p><Link href={"/strategies/long-put"} className="button">学习 Long Put策略 &rarr;</Link></p>
        </div>
        <div className={styles.greekCard}>
          <h3><Link href={"/strategies/covered-call"}>备兑看涨期权 (Covered Call)</Link></h3>
          <p>如果你手里有股票，并觉得它短期内不会大涨，可以试试这个策略。它就像把股票“租”出去，帮你持续创造额外收入。</p>
          <p><strong>适合场景：</strong>持有股票，对后市温和看涨或中性，希望增加额外收益。</p>
          <p><Link href={"/strategies/covered-call"} className="button">学习 Covered Call策略 &rarr;</Link></p>
        </div>
        <div className={styles.greekCard}>
          <h3><Link href={"/strategies/protective-put"}>保护性看跌期权 (Protective Put)</Link></h3>
          <p>担心你持有的股票会突然下跌？这个策略就是为你量身定做的“资产保险”，它能在不放弃未来上涨潜力的前提下，保护你的资产。</p>
          <p><strong>适合场景：</strong>持有股票，但担心短期内出现回调或黑天鹅事件。</p>
          <p><Link href={"/strategies/protective-put"} className="button">学习 Protective Put策略 &rarr;</Link></p>
        </div>
      </div>

      <h2>第四步：探索更多</h2>
      <p className={styles.muted}>当你掌握了这四个基础后，就可以开始探索更广阔的期权世界了。利用本站工具，你可以：</p>
      <div className={styles.flexLinks}>
        <Link href="/strategies"><button>进入策略库</button></Link>
        <Link href="/compare"><button>对比不同策略</button></Link>
        <Link href="/builder"><button>自由构建你的策略</button></Link>
      </div>
    </section>
  )
}